...
export async function POST(req: NextRequest) {
  try {
    ...
    if (role === "emergency_contact") {
      ...
      // (36) Persist the full message to users/{sendToUid}/fromContactVoiceMessages/{AUTO_ID}
      const newMessageRef = db.collection(`users/${sendToUid}/fromContactVoiceMessages`).doc();
...
export async function GET(req: NextRequest) {
  try {
    ...
    const snapshot = await db
      .collection(`users/${callerUid}/fromContactVoiceMessages`)
