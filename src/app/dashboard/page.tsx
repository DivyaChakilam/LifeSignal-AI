...
interface UserDoc {
  ...
  phone?: string;
  // Preferred notification behavior for this user
  notificationMethod?: "push" | "push_then_call" | "call_immediately";
  // Delay (in minutes) before calling when using push-then-call
  pushThenCallDelayMinutes?: number;
  // Number of push notifications if using "Push Only"
  pushOnlyCount?: number;
  // Number of push notifications before calling when using "Push → Then Call"
...
export default function DashboardPage() {
  ...
  // For scrolling to App Settings section
...
const scrollToSettings = () => {
  ...
  // User's preferred alert method (push, push-then-call, or call immediately)
  const [notificationMethod, setNotificationMethod] = useState<string>("push"); 
  // Delay (in minutes) before calling when using push-then-call
  const [pushThenCallDelayMinutes, setPushThenCallDelayMinutes] = useState<number>(1); 
  // Number of push notifications to send when the user selects "Push Only"
  const [pushOnlyCount, setPushOnlyCount] = useState<number>(3);
  // Number of push notifications to send before calling when "Push → Then Call" is selected
  const [pushThenCallCount, setPushThenCallCount] = useState<number>(3);
  ...
  useEffect(() => {
    const offAuth = onAuthStateChanged(auth, async (user) => {
      ...
      const unsub = onSnapshot(uref, (snap) => {
        ...
        if (typeof data.notificationMethod === "string") {
          setNotificationMethod(data.notificationMethod);
        } else {
         setNotificationMethod("push");
        }

        if (typeof data.pushThenCallDelayMinutes === "number") {
          setPushThenCallDelayMinutes(data.pushThenCallDelayMinutes);
        } else {
          setPushThenCallDelayMinutes(1);
        }

        // Load user's Push Only count from Firestore (fallback to 3 if unset)
        if (typeof data.pushOnlyCount === "number") {
          setPushOnlyCount(data.pushOnlyCount);
        } else {
          setPushOnlyCount(3);
        }

        // Load user's Push → Then Call count from Firestore (fallback to 3 if unset)
        if (typeof data.pushThenCallCount === "number") {
          setPushThenCallCount(data.pushThenCallCount);
        } else {
           setPushThenCallCount(3);
        }
  ...
  const openContactDialog = useCallback(
    async (kind: ContactKind) => {
      ...
      try {
        const q = fsQuery(
          collection(db, `users/${mainUserUid}/fromContactVoiceMessages`),
    ...
    <>
      ...
      <div className="flex flex-col min-h-screen bg-secondary overflow-x-hidden">
        ...
        <main className="flex-grow container mx-auto px-4 pt-24 sm:pt-8 pb-8">
          ...
          {/* Scroll button for App Settings */}
          <Button
           size="sm"
           variant="outline"
  ...
  <Button
    variant="default"
    onClick={() => router.push("/mainuser-settings")}
  >
    Personal Settings
  </Button>
                ...
                <CardContent className="space-y-6">
                  ...
                  <section className="space-y-4">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">
                        Choose how you want LifeSignal to notify you during check-ins and escalations.
                      </p>
                    </div>
                    ...
                    <div className="space-y-2">
                        ...
                        onValueChange={async (value) => {
                          ...
                          try {
                            await updateDoc(userRef.current, {
                              ...
                              // Clear delay if switching to non-delay methods
                              ...(value !== "push_then_call" && { pushThenCallDelayMinutes: deleteField() }),
                            });
                            ...
                            toast({
                              ...
                              description:
                                value === "push"
                                  ? "You will receive push notifications only."
                                  : value === "call_immediately"
                                  ? "You will receive an immediate call for alerts."
                                  : "You will get a push notification first, then a call if no response.",
                            });
                          } catch (error) {
                            toast({
                              title: "Update failed",
                              description: "Unable to save notification preference.",
                              variant: "destructive",
                            });
                          }
                        }}
                      ...
                      >
                        ...
                        <SelectContent>
                          ...
                          <SelectItem value="push_then_call">Push → Then Call</SelectItem>
                          <SelectItem value="call_immediately">Call Immediately</SelectItem>
                        </SelectContent>
                      </Select>
                    </div>
                    {/* Push Count (Only for Push Only) */}
{notificationMethod === "push" && (
  <div className="space-y-2">
    <Label>Number of Push Notifications</Label>
    <Select
      onValueChange={async (value) => {
        if (!userRef.current) return;
        ...
        try {
          await updateDoc(userRef.current, {
            pushOnlyCount: Number(value),
          });

          toast({
            title: "Push count updated",
            description: `You will receive ${value} push notification(s).`,
          });
        } catch {
          toast({
            title: "Update failed",
            description: "Unable to save push count.",
            variant: "destructive",
          });
        }
      }}
      value={String(pushOnlyCount || 3)}
    >
      <SelectTrigger className="w-full">
        <SelectValue placeholder="Select number of push notifications" />
      </SelectTrigger>
      <SelectContent>
        {[1, 2, 3, 4, 5].map((n) => (
...
{/* Push Count Before Call (Only for Push → Then Call) */}
{notificationMethod === "push_then_call" && (
  <div className="space-y-2">
    <Label>Push Notifications Before Call</Label>
    <Select
      onValueChange={async (value) => {
        if (!userRef.current) return;
        ...
        try {
          await updateDoc(userRef.current, {
            pushThenCallCount: Number(value),
          });

          toast({
            title: "Push-before-call count updated",
            description: `The app will send ${value} push notification(s) before calling.`,
          });
        } catch {
          toast({
            title: "Update failed",
            description: "Unable to save push-before-call count.",
            variant: "destructive",
          });
        }
      }}
      value={String(pushThenCallCount || 3)}
    >
      <SelectTrigger className="w-full">
        <SelectValue placeholder="Select number of push notifications before call" />
      </SelectTrigger>
      <SelectContent>
        {[1, 2, 3, 4, 5].map((n) => (
          <SelectItem key={n} value={String(n)}>
            {n} push{n > 1 ? "es" : ""}
                    ...
                    {/* Delay Dropdown (Only if push_then_call selected) */}
                    {notificationMethod === "push_then_call" && (
                      <div className="space-y-2">
                        ...
                        <Select
                          onValueChange={async (value) => {
                            ...
                            try {
                              await updateDoc(userRef.current, {
                                pushThenCallDelayMinutes: Number(value),
                              });
                  
                              toast({
                                title: "Delay updated",
                                description: `We will wait ${value} minute(s) after the push notification before calling.`,
                              });
                            } catch {
                              toast({
                                title: "Update failed",
                                description: "Unable to save delay.",
                                variant: "destructive",
                              });
                            }
                          }}
                          value={String(pushThenCallDelayMinutes || 1)}
                        >
                          <SelectTrigger className="w-full">
                    ...
                    {/* Info text */}
                    <p className="text-xs text-muted-foreground">
                      If you do not respond, LifeSignal will escalate automatically and notify your emergency contacts based on your configuration.
                    </p>
                  </section>
                </CardContent>
              </Card>
